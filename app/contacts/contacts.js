    ...
    function ($stateProvider,   $urlRouterProvider) {
      $stateProvider
        ...
        .state('contacts', {
          ...
          //设置为true,表示这个状态不能明确被激活，它可以由激活的子状态隐式的激活
          abstract: true,

          // This abstract state will prepend '/contacts' onto the urls of all its children.
          //这种抽象的状态会在/contacts到URL的所有孩子.
          url: '/contacts',

          // Example of loading a template from a file. This is also a top level state,
          // so this template file will be loaded and then inserted into the ui-view
          // within index.html.
          //装载一个模板文件，这是个Top级别的状态，因此模板文件将被插入到index.html的ui-view
          ...
          //在控制器被实例化之前。使用的解决resolve来解决任何异步控制器依赖关系。 
          resolve: {
            contacts: ['contacts',
              function( contacts){
                return contacts.all();
        ...
        .state('contacts.list', {
          ...
          //注意：现在这个状态不是Top级别的状态，它的模板将插入到父状态模板的ui-view。
          //因此，它插入到contacts.html的ui-view
        ...
        .state('contacts.detail', {
          ...
          //如果父模板中有多个ui-view或者目标为状态树的更高级，那么需要使用视图对象配置多视图
          //每个视图都可以有自己的模板、控制器和解决数据 resolve data

          // View names can be relative or absolute. Relative view names do not use an '@'
          // symbol. They always refer to views within this state's parent template.
          // Absolute view names use a '@' symbol to distinguish the view and the state.
          // So 'foo@bar' means the ui-view named 'foo' within the 'bar' state's template.
          //视图名称可以是相对或者绝对的。相对视图名称不使用@符号。它们总是指向状态的父模板中的视图
          //绝对视图名称使用@符号来区别视图和状态。故此foo@bar指的是在bar状态的模板内命名为foo的ui-view
          views: {

            // So this one is targeting the unnamed view within the parent state's template.
            //在父状态模板（contacts）内没有命名的视图ui-view
            '': {
            ...
            //根视图无命名状态的hint视图名称，它在index.html
            //这表明了如何在*任何*祖先的状态的任何视图中填充模板
            'hint@': {
              template: 'This is contacts.detail populating the "hint" ui-view'
            },

            // This one is targeting the ui-view="menuTip" within the parent state's template.
            //父状态模板内视图名称为menuTip的视图，ui-view="menuTip"
            'menuTip': {
        ...
        .state('contacts.detail.item', {
          ...
          views: {
            ...
            //这个视图的目标父状态contact.detail中为未命名ui-view
            //如果我们不想在下面设置“提示”视图，我们就不必这样做了。
            //我们可以在视图对象外面设置templateUrl和controller
            '': {
        ...
        .state('contacts.detail.item.edit', {
          views: {
            ...
            //这个目标是状态contacts.detail之内无命名ui-view
            '@contacts.detail': {
